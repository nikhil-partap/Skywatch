...
class WeatherApp {
  ...
  async getCurrentLocation() {
    ...
    try {
      ...
      const cityName = this.currentWeather
      ...
      if (error.code === 1) {
        errorMessage =
          "Location access denied. Please allow location access or search for a city instead.";
        this.showToast("Location access denied", "error");
      } else if (error.code === 2) {
        errorMessage =
          "Location unavailable. Please try again or search for a city instead.";
        this.showToast("Location unavailable", "error");
      } else if (error.code === 3) {
        errorMessage =
          "Location request timed out. Please try again or search for a city instead.";
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400) {
          ...
          throw new Error(
            errorData.details || "Server error. Please try again later."
          );
          ...
          throw new Error(
            errorData.details || "Request timeout. Please try again."
          );
        } else {
          throw new Error(
            errorData.details ||
  ...
  updateCurrentWeather() {
    ...
    this.weatherIcon.className = `weather-icon ${this.getWeatherIcon(
    ...
    this.animateValue(
      this.windSpeed,
      this.convertWindSpeed(weather.wind.speed)
    );
    this.animateValue(
      this.visibility,
  ...
  createHourlyCard(hour) {
    ...
    card.innerHTML = `
            ...
            <i class="hourly-icon ${this.getWeatherIcon(
              hour.weather[0].id
            )}"></i>
  ...
  updateAdditionalInfo() {
    ...
    this.sunrise.textContent = this.formatTime(
      weather.sys.sunrise,
      weather.timezone
    );
    this.sunset.textContent = this.formatTime(
      weather.sys.sunset,
      weather.timezone
    );
